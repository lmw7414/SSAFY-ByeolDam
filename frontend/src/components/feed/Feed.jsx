    ...
    const renderComments = () => {
      if(showAllComments) {
        return comments.map(({ id, nickName, content, createdAt, parentId, childrenComments }) => {
         ...
         return limitedComments.map(({ id, nickName, content, createdAt, parentId, childrenComments }) => {
             return (
                 <Comment
                     key={id}
                     id={id}
        ...
        <div className='feed-content-container'>
          ...
          <div className='feed-like'>
              <div onClick={toggleLike} className='like-box'>
                {like ? <img src='/images/Layer_1.png' alt='Liked'/> : <img src='/images/empty_heart.png' alt='Not Liked'/>}
              </div>
          ...
          <div className='comment-container'>
            ...
            {comments.length > 2 && !showAllComments && (
              <div onClick={() => setShowAllComments(true)} style={{color:'#c7bed6'}}>댓글 모두 보기</div>
            )}
          </div>
